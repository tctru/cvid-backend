app.controller('bannercatsController', ['$scope', '$http', '$rootScope', '$routeParams','$location','ngDialog','$route','FileUploader',
    function ($scope, $http, $rootScope,$routeParams, $location,ngDialog,$route,FileUploader) {
        ...
        $scope.openaddproducttypes = function (size) {
          $location.path("/administrator/newbannercats");
        };
        ...
        $scope.savebannercats = function () {
                ...
                $http({
                    method: "post",
                    url:'/bannercats/addbannercatsres',
                    data: $.param(Employee),
                    ...
                    if(result["result"]==0){
                        ...
                        $location.path("/administrator/listbannercats");
                    }       
                })
        ...
        $scope.delnewpages = function(index) {
            ...
            if (confirm("Bạn có muốn xóa coonh ty này ko?")) {
              $http({
                  method: "post",
                  url:'/savebannercats/delbannercats',
                  data: $.param({id:todel}),
                  headers: { 'Content-Type': 'application/x-www-form-urlencoded'}
              }).success(function(result, status, headers, config) {
                //alert(result);
...
app.controller('editbannercatsController', ['$scope', '$http', '$rootScope', '$routeParams','$location','ngDialog','$route','FileUploader',
    function ($scope, $http, $rootScope,$routeParams, $location,ngDialog,$route,FileUploader) {
        // this configuration depends on your site.
        //debugger;
        //
        ...
        $scope.divadd = true;
        $http.get('/bannercats/getnewscatinfo?id='+$routeParams['id']).success(function(data) {
            //$route.reload();
        ...
        $scope.saveeditbannercats = function () {
                var Employee = {
                ...
                $http({
                    method: "post",
                    url:'/bannercats/editbannercatsres',
                    data: $.param(Employee),
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded'}
                }).success(function(result, status, headers, config) {
                    if(result["result"]==0){
